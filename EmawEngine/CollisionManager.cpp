...
bool CollisionManager::checkVMapCollision(BoxCollidable* obj, VoxelMap* map)
{
	...
	int i = (int)(curPos.x - dimensions.x * 2) % CHUNK_SIZE;
	int j = (int)(curPos.y - dimensions.y * 2) % CHUNK_HEIGHT;
	int k = (int)(curPos.z - dimensions.z * 2) % CHUNK_SIZE;
	if (i >= 0 && j >= 0 && k >= 0)
	{
		for (i; i <= (int)(curPos.x + dimensions.x * 2) % (CHUNK_SIZE + 1); i++)
		{
			for (j; j <= (int)(curPos.y + dimensions.y * 2) % (CHUNK_HEIGHT + 1); j++)
			{
				for (k; k <= (int)(curPos.z + dimensions.z * 2) % (CHUNK_SIZE + 1); k++)
				{
